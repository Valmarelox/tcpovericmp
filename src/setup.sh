#!/bin/bash
set -ex
# Add a namespace per network entity
ip netns add client
...
# Our tunneler sniff TCP in raw, so we wan't to stop them from processing TCP packets normaly
# e.g don't send unreachables or resets
...
# Same for ICMP, drop ICMP packets which will mess our protocol
